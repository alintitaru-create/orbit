...
const Pubblica={
  ...
  /* Neutralizza il testo scritto da voi prima di infilarlo nella pagina:
     una didascalia che contenesse parentesi angolari verrebbe altrimenti
     interpretata come codice invece che letta come testo. */
  testo(s){ return String(s||'').replace(/[<>&"]/g,c=>({'<':'&lt;','>':'&gt;','&':'&amp;','"':'&quot;'}[c])); },
    ...
    Media.all(day).then(items=>{
      ...
      box.innerHTML=`
        ...
        ${items.length?`<div class="pub-scelta">${items.map((m,k)=>
          `<label><input type="checkbox" value="${m.id}" checked> ${m.type==='video'?'video':'foto'} ${k+1}${m.caption?' · '+this.testo(m.caption.slice(0,26)):''}</label>`
